...
public class JalaliDateTime {
    ...
    public JalaliDateTime(long year, int month, int day, int hour, int minute, int second, int microsecond) {
        /*
         * Constructor
        ...
        if (year <= 0) {
            throw new DAexception(".سال مورد قبول بین یک و 9223372036854775807 است");
        } else if ((month > 12) || (month < 1)) {
            throw new DAexception(".برج های سال خورشیدی بین 1 و 12 است");
        } else if ((day < 1) || (day > DAConvertDate.parsiMonthDaysCount(year, month))) {
            throw new DAexception("روز برج" + " " + DAConvertDate.parsiMonthName[month] + " سال " + year + " بین عدد 1 و  " + DAConvertDate.parsiMonthDaysCount(year, month) + " است ");
    ...
    public String getMonthName() {
        return DAConvertDate.parsiMonthName[this.month];
    }

    // شماره روز هفته تاریخ مورد نظر
    public int getDayOfWeekNumber() {
        return DAConvertDate.dayOfWeekNumber(this);
    }

    // نام روز هفته تاریخ مورد نظر
    public String getDayOfWeekName() {
        return DAConvertDate.dayOfWeekName(this);
    }
    ...
    public JalaliDateTime addDays(long days) {
        ...
        var gregorian = DAConvertDate.convertToGregorian(this);
        var updatedGregorian = gregorian.plus(Duration.ofDays(days));
        return DAConvertDate.convertToParsiDate(updatedGregorian);
    }
