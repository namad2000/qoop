...
class OutboxWriterTest {
    ...
    void shouldPublishMessageAndMarkAsSentSuccessfully() throws Exception {
        ...
        when(repository.markAsSent(eq(eventId), any(LocalDateTime.class), eq(0L))).thenReturn(1);
        // Use isNull() with explicit casting to resolve overload ambiguity
        ...
        verify(repository, times(1)).markAsSent(eq(eventId), any(LocalDateTime.class), eq(0L));
    ...
    void shouldHandleOptimisticLockFailure() throws Exception {
        ...
        doNothing().when(publisher).publish(eq("test-topic"), (List<Header>) isNull(), any());
        when(repository.markAsSent(eq(eventId), any(LocalDateTime.class), eq(0L))).thenReturn(0);
    ...
    void shouldHandlePublisherException() throws Exception {
        ...
        // Use explicit casting to resolve overload ambiguity
        ...
        verify(publisher, times(1)).publish(eq("test-topic"), (List<Header>) isNull(), any());
        verify(repository, never()).markAsSent(any(), any(), any());
        verify(outboxErrorLogService, times(1)).handleFailure(eq(eventId), eq(0L), any());
    }
    ...
    void shouldProcessMultipleMessagesInChunk() throws Exception {
        ...
        when(repository.markAsSent(eq(eventId1), any(LocalDateTime.class), eq(0L))).thenReturn(1);
        when(repository.markAsSent(eq(eventId2), any(LocalDateTime.class), eq(1L))).thenReturn(1);
        ...
        verify(repository, times(2)).markAsSent(any(), any(), any());
    }
